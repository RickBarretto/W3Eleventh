...
def on_acceptance(on_proposal, trades):
...
def verify_trade_accepted(trader_a, trader_b, on_acceptance, decks):
    trader_a, card_x = trader_a
    trader_b, card_y = trader_b

    assert card_x in decks.deck_of(trader_b)
    assert card_x not in decks.deck_of(trader_a)

    assert card_y in decks.deck_of(trader_a)
    assert card_y not in decks.deck_of(trader_b)
