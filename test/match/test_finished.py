...
def one_squad_set(matches, context):
    match_id = context["match_id"]
    host = context["host"]

    with boa.env.prank(host):
        matches.choose_squad(match_id, b"alice-ready")

    context["winner"] = host


@when("the second player chooses its squad")
def second_squad_and_result(matches, context):
    match_id = context["match_id"]
    host = context["host"]
    guest = context["guest"]
    ...
    with boa.env.prank(guest):
        matches.choose_squad(match_id, guest_squad)
...
def winner_recorded(matches, context):
    match_id = context["match_id"]
    *_, winner = matches.matches(match_id)
    assert winner == context["winner"]


@then("the status is set to 2 (completed)")
def status_completed(matches, context):
    match_id = context["match_id"]
    _, _, status, *_ = matches.matches(match_id)
    assert status == 2
